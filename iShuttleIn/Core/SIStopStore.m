...
typedef NS_ENUM(NSUInteger, HalfDay) {
    AM,
    PM
};
    ...
    for (NSDictionary *route in self.routesStops) {
        if ([[route valueForKey:@"ID"] longValue] == [routeId longValue]) {
            ...
            for (int i=0; i<patterns.count; i++) {
                ...
                HalfDay amOrPm = [self getHalfDay];
                NSString *direction = [pattern objectForKey:@"Direction"];
                if (([direction isEqualToString:@"Inbound"] && amOrPm == PM) || ([direction isEqualToString:@"Outbound"] && amOrPm == AM)) {
